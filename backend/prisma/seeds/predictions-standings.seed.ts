...
export async function seedPredictions(users: any[], matches: any[]) {
  console.log('\n🔮 Creating predictions...');

  const matchesForPrediction = matches.filter(m => m.status === 'FINISHED' || m.status === 'LIVE');
  ...
  for (const match of matchesForPrediction) {
    for (let i = 0; i < users.length; i++) {
      ...
      if (Math.random() < accuracy && match.homeScore !== null && match.awayScore !== null) {
        predHome = match.homeScore + (Math.random() > 0.5 ? 1 : -1) * Math.floor(Math.random() * 2);
        predAway = match.awayScore + (Math.random() > 0.5 ? 1 : -1) * Math.floor(Math.random() * 2);
      ...
      const predictedResult = predHome > predAway ? 'WIN_HOME' : predHome < predAway ? 'WIN_AWAY' : 'DRAW';
      const actualResult = match.homeScore! > match.awayScore! ? 'WIN_HOME' : match.homeScore! < match.awayScore! ? 'WIN_AWAY' : 'DRAW';

      const isExactScore = match.status === 'FINISHED' && predHome === match.homeScore && predAway === match.awayScore;
      const isCorrectResult = match.status === 'FINISHED' && predictedResult === actualResult;
...
export async function seedStandings(leagues: any, teams: any) {
  console.log('\n📊 Creating standings...');

  // Helper function to generate realistic standings
  const generateStandings = (teamsList: any[], leagueId: number) => {
    ...
    for (let i = 0; i < totalTeams; i++) {
      ...
      // Top teams: more wins
      // Mid teams: balanced
      // Bottom teams: more losses
      let won, drawn, lost, gf, ga;
  ...
  const laLigaStandings = generateStandings(teams.laLigaTeams, leagues.laLiga.id);
  const bundesligaStandings = generateStandings(teams.bundesligaTeams, leagues.bundesliga.id);
  ...
  console.log(`✅ Created ${totalCount} standings entries (PL: ${plStandings.length}, La Liga: ${laLigaStandings.length}, Bundesliga: ${bundesligaStandings.length})`);
}
